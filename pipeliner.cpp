...
//    faceWrapper_.reset();
    ...
    LOG(WARNING) << "[Pipeliner] dtor" << endl;
...
Pipeliner::init(ptr_lib::shared_ptr<FrameBuffer> frameBuffer, ptr_lib::shared_ptr<FaceWrapper> faceWrapper)
{
    ...
    switchToState(Pipeliner::State::Ready);
}
...
Pipeliner::stop()
{
    LOG(INFO) << "[Pipeliner] Stoping" << endl;

    window_.reset();
    while(getState() != Stoped)
    {
        switchToState(Stoped);
    }
}
