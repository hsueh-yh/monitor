...
isStdOutActive_(true)
{
    ...
    getOutStream() << "Log line format: \nTimeStamp\t[level]["
                   << std::setw(27) << "log-object       ]-"
                   << std::setw(12) << "threadId"
                   << ": msg..."
                   << std::endl;
    isProcessing_ = true;
}
                     ...
                     const int& locationLine)
{
        ...
        logType >= (NdnLogType)logLevel_)
    {
        ...
        if (//logType < (NdnLogType)NdnLoggerLevelDebug &&
            ...
            locationLine >= 0)
        {
            size_t loc = locationFile.rfind("/") + 1;
            std::string logLocation = locationFile.substr(loc,locationFile.size()-loc);
            getOutStream() << "("
                           << std::setw(17) << logLocation << ":"
                           << std::setw(5) << locationLine << ") ";
        }
        */
        currentLogRecord_ << ": ";
        //LOG(INFO) << "logger.log" << std::endl;
    }
